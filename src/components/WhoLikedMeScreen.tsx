...
import { Heart, Crown, Star, Sparkles, Lock } from 'lucide-react';
...
// Limite visible selon le tier (PDF)
...
export default function WhoLikedMeScreen({
  ...
  async function loadData() {
    try {
      ...
      // Compter le total des likes reçus
      ...
      // Récupérer les derniers likes avec infos du profil
      const limit = getVisibleLimit(tier);
      if (limit === 0) {
        // Free : on récupère quand même les 3 derniers pour les afficher flous
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-32 pt-4">
        ...
        {!loading && totalCount > 0 && (
          <>
            ...
            {!isPremium && totalCount > 0 && (
              <div className="bg-gradient-to-r from-rose-500 to-amber-500 rounded-2xl p-5 mb-6 text-white">
                ...
                <button
                  ...
                  className="w-full py-2.5 bg-white text-rose-600 rounded-xl font-semibold text-sm hover:bg-rose-50 transition-colors"
                >
            ...
            {/* Bannière upgrade si essentiel ou elite */}
            {isPremium && visibleLimit < 999 && totalCount > visibleLimit && (
              <div className="bg-gradient-to-r from-amber-50 to-rose-50 dark:from-amber-900/20 dark:to-rose-900/20 border border-amber-200 dark:border-amber-800 rounded-2xl p-4 mb-5 flex items-center gap-3">
                ...
                <button
                  onClick={onOpenPremium}
                  className="px-3 py-2 bg-gradient-to-r from-rose-500 to-amber-500 text-white rounded-xl text-xs font-semibold"
                >
                  Upgrade
                </button>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              {/* Cartes fantômes pour les likes cachés (utilisateurs free) */}
